...
sub process_app_info_ajax_content{
	...
	foreach my $tr(@tr_tags){
		...
		push @platforms, $td_tags[2]->as_text if ref $td_tags[2] eq 'HTML::Element';
		push @versions, $td_tags[3]->as_text  if ref $td_tags[3];
        push @file_size,$td_tags[4]->as_text  if ref $td_tags[4];
		push @down_urls, $td_tags[5]->as_text if ref $td_tags[5];
	}
	$app_info->{current_version} = $versions[0];
    $app_info->{size} = $file_size[0];
    my $platform_str = join " " ,@platforms;
    my @version = ($platform_str =~ /Android\s([^A]+)/);
    $app_info->{min_os_version} = min @version;
    $app_info->{max_os_version} = max @version;
	$app_info->{apk_url} = $down_urls[0] if $app_info->{price} ne "0";
}
sub get_login {
    my $res_login = $ua->get(
"http://www.189store.com/index.php?app=ajax&act=getUserInfo&js=1&ReturnURL="
    ...
    if ( $res_login->is_success ) {
        if ( $res_login->content =~ /登出/m ) {
            $has_login = 1;
        }elsif( $res_login->content =~ /value="?([^"]+)"?>/ && !$has_login) {
            $login_key = $1;
        }
    }
